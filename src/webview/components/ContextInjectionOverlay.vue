    ...
    <template #header-actions>
      <template v-if="store.activeTab === 'recall' && trajectory">
        ...
        <Badge
          ...
          variant="outline"
          class="text-xs border-emerald-500/50 text-emerald-400"
        >
        ...
        <Badge
          ...
          variant="outline"
          class="text-xs border-red-500/50 text-red-400"
        >
      ...
      <div v-else class="space-y-3">
        ...
        <div v-if="showTabs" class="flex gap-1 mb-3 border-b border-border pb-2">
          <button
            ...
            type="button"
            class="px-3 py-1 rounded-md text-xs font-medium transition-all duration-150 cursor-pointer border"
        ...
        <!-- Tab description -->
        <p v-if="tabDescription && hasAnyData" class="text-xs text-muted-foreground/70 leading-relaxed -mt-1 mb-2">
          {{ tabDescription }}
        </p>
        ...
        <div v-if="store.activeTab === 'recall'" class="space-y-3">
          ...
          <div v-if="trajectory" class="mb-3 rounded-lg bg-muted/80 border border-border px-3 py-2">
            ...
            <span class="text-xs text-foreground/80">{{ trajectory.userPrompt }}</span>
          </div>

          <!-- Metadata row -->
          <div v-if="trajectory" class="mb-3 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <div v-if="displayIterations.length > 0" class="space-y-3">
            ...
            >
              ...
              >
                <div class="flex items-center gap-2 min-w-0">
                  <Badge variant="secondary" class="text-xs px-1.5 py-0 shrink-0">
                    #{{ iter.index + 1 }}
                  </Badge>
                  <span class="text-xs text-foreground truncate">{{ iter.modelResponse.slice(0, 120) }}{{ iter.modelResponse.length > 120 ? '...' : '' }}</span>
                </div>
                <div class="flex items-center gap-1.5 shrink-0">
                  <Badge v-if="iter.subcalls.length > 0" variant="outline" class="text-xs px-1 py-0 border-primary/30 text-primary/70">
                    {{ t('contextInjection.recallSubcalls', { count: iter.subcalls.length }) }}
                  </Badge>
                  <span class="text-xs text-muted-foreground tabular-nums">{{ formatDuration(iter.durationMs) }}</span>
                </div>
              </button>
              ...
              <div v-if="expandedIterations.has(iter.index)" class="px-3 pb-3 space-y-2 border-t border-border/50">
                ...
                <Collapsible :default-open="true" class="mt-2">
                  <CollapsibleTrigger class="group flex items-center gap-1.5 w-full text-left cursor-pointer">
                    <IconChevronDown :size="12" class="shrink-0 text-muted-foreground transition-transform -rotate-90 group-data-[state=open]:rotate-0" />
                    <span class="text-xs font-medium text-muted-foreground uppercase tracking-wider">{{ t('contextInjection.recallModelResponse') }}</span>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <div class="mt-1 text-xs text-foreground/80 bg-background rounded-lg p-2">
                ...
                <Collapsible v-if="iter.codeBlock" :default-open="true">
                  <CollapsibleTrigger class="group flex items-center gap-1.5 w-full text-left cursor-pointer">
                    <IconChevronDown :size="12" class="shrink-0 text-muted-foreground transition-transform -rotate-90 group-data-[state=open]:rotate-0" />
                    <span class="text-xs font-medium text-muted-foreground uppercase tracking-wider">{{ t('contextInjection.recallCodeBlock') }}</span>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <div class="mt-1">
                ...
                <Collapsible v-if="iter.replOutput" :default-open="true">
                  <CollapsibleTrigger class="group flex items-center gap-1.5 w-full text-left cursor-pointer">
                    <IconChevronDown :size="12" class="shrink-0 text-muted-foreground transition-transform -rotate-90 group-data-[state=open]:rotate-0" />
                    <span class="text-xs font-medium text-muted-foreground uppercase tracking-wider">{{ t('contextInjection.recallReplOutput') }}</span>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <div class="mt-1">
                ...
                <div v-if="iter.subcalls.length > 0" class="space-y-2">
                  ...
                  >
                    <div class="flex items-center justify-between gap-2">
                      <span class="text-xs font-mono text-muted-foreground truncate">{{ sub.model }}</span>
                      <span class="text-xs text-muted-foreground tabular-nums shrink-0">{{ formatDuration(sub.durationMs) }}</span>
                    </div>

                    <Collapsible>
                      <CollapsibleTrigger class="group flex items-center gap-1.5 w-full text-left cursor-pointer">
                        <IconChevronDown :size="12" class="shrink-0 text-muted-foreground transition-transform -rotate-90 group-data-[state=open]:rotate-0" />
                        <span class="text-xs font-medium text-muted-foreground uppercase tracking-wider">{{ t('contextInjection.recallSubcallPrompt') }}</span>
                        <span class="text-xs text-muted-foreground/50 truncate">{{ sub.prompt.slice(0, 80) }}</span>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                    ...
                    <Collapsible :default-open="true">
                      <CollapsibleTrigger class="group flex items-center gap-1.5 w-full text-left cursor-pointer">
                        <IconChevronDown :size="12" class="shrink-0 text-muted-foreground transition-transform -rotate-90 group-data-[state=open]:rotate-0" />
                        <span class="text-xs font-medium text-primary/70 uppercase tracking-wider">{{ t('contextInjection.recallSubcallResponse') }}</span>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
            ...
            <div v-if="isRecallStreaming" class="flex items-center justify-center gap-2 py-3">
              <LoadingSpinner :size="14" />
              <span class="text-xs text-muted-foreground">{{ t('contextInjection.recallRunning', { n: store.liveIterations.length + 1 }) }}</span>
            </div>
        ...
        <div v-else-if="store.activeTab === 'nodeContext' && trajectory" class="space-y-3">
          ...
          <div class="flex items-center gap-2 mb-3">
            <Badge
              v-if="trajectory.nodeTitle"
              variant="outline"
              class="text-xs border-primary/30 text-primary"
            >
              {{ trajectory.nodeTitle }}
            </Badge>
            <span v-if="!trajectory.nodeTitle" class="text-xs text-muted-foreground">{{ t('contextInjection.noNodeAssigned') }}</span>
          ...
          <template v-if="store.contextViewMode === 'cards'">
            ...
            <div v-if="trajectoryNode" class="space-y-3 mb-4">
              ...
              <div v-if="trajectoryNode.summary" class="rounded-lg border border-border bg-muted/60 p-3 space-y-2">
                <p class="text-xs text-foreground/80">{{ trajectoryNode.summary.taskDescription }}</p>
                <div v-if="trajectoryNode.summary.filesChanged.length > 0" class="text-xs text-muted-foreground">
            ...
            <Collapsible v-if="trajectory.seedContext">
              <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 mb-3">
                <CollapsibleTrigger class="group flex items-center gap-2 w-full px-3 py-2 cursor-pointer">
                  ...
                  <span class="text-xs text-muted-foreground/50">{{ trajectory.seedContext.length.toLocaleString() }} chars</span>
                </CollapsibleTrigger>
                <CollapsibleContent>
            ...
            <Collapsible v-if="trajectory.relatedSummaries.length > 0">
              <div class="rounded-lg border border-primary/20 bg-primary/5 mb-3">
                <CollapsibleTrigger class="group flex items-center gap-2 w-full px-3 py-2 cursor-pointer">
                  ...
                  <Badge variant="secondary" class="text-xs px-1.5 py-0">{{ trajectory.relatedSummaries.length }}</Badge>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <div class="px-3 pb-3 space-y-1.5">
                    ...
                    >
                      <div class="flex items-center gap-2">
                        <span class="text-xs font-medium text-foreground/80">{{ rs.title }}</span>
                        <Badge
                          variant="outline"
                      ...
                      <p class="text-xs text-foreground/70">{{ rs.taskDescription }}</p>
                      <div v-if="rs.filesChanged.length > 0" class="text-xs text-muted-foreground/60">
                        {{ t('nodeOverlay.files') }} {{ rs.filesChanged.join(', ') }}
                      </div>
                      <div v-if="rs.keyDecisions.length > 0" class="space-y-0.5">
                        <span class="text-xs font-medium text-muted-foreground/60">{{ t('nodeOverlay.keyDecisions') }}</span>
                        <ul class="list-disc list-inside text-xs text-foreground/60 space-y-0.5 pl-1">
            ...
            <div v-if="trajectory.contextTurns.length > 0" class="space-y-3">
              <div class="flex items-center gap-2 mb-2">
                ...
                <Badge variant="secondary" class="text-xs px-1.5 py-0">{{ t('nodeOverlay.turnsBadge', { count: trajectory.contextTurns.length }) }}</Badge>
                <div class="h-px flex-1 bg-border" />
              </div>
            ...
            <div v-else-if="trajectory.finalContext" class="text-xs text-muted-foreground">
