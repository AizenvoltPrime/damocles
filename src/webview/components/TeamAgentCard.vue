...
const canCancel = computed(() => {
  ...
  const agentActive = props.agent.status === 'running' || props.agent.status === 'pending';
  ...
  <Card class="text-sm overflow-hidden cursor-pointer hover:bg-foreground/[0.02] transition-colors" @click="openAgentDetail">
    ...
    <CardContent class="px-3 py-2.5 space-y-2">
      <div class="flex items-center justify-between">
        ...
        <Badge variant="secondary" :class="badgeClass" class="gap-1">
          <LoadingSpinner v-if="agent.status === 'running'" :size="10" />
          <IconCheck v-else-if="agent.status === 'completed'" :size="10" />
          <IconXCircle v-else-if="agent.status === 'failed'" :size="10" />
          <IconBan v-else-if="agent.status === 'cancelled'" :size="10" />
          <span>{{ t('team.status.' + agent.status) }}</span>
        </Badge>
      </div>
