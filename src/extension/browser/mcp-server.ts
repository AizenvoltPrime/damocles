...
function imageResult(base64: string, text?: string) {
...
function errorResult(toolName: string, error: unknown): { content: ContentBlock[]; isError: true } {
...
export function createBrowserMcpServer(
  ...
  async function screenshotAfter(cdp: ReturnType<typeof requireCdp>, delayMs: number, text: string) {
  ...
  async function screenshotWithSnapshot(cdp: ReturnType<typeof requireCdp>, delayMs: number, text: string) {
    await new Promise(r => setTimeout(r, delayMs));
  ...
  return createSdkMcpServer({
    name: 'damocles-browser',
    version: '1.0.0',
    tools: [
      tool(
        'browser_open',
        "Open a URL in VS Code's integrated browser with full automation. User sees the browser live. If already open, navigates to the new URL. Provide exact, complete URLs — look up routes in the codebase rather than guessing. After opening, call browser_query to discover interactive elements on the page.",
        {
          url: z.string().describe('Exact full URL to open'),
        },
        async (input) => {
          try {
            await browserService.open(input.url);
            const cdpReady = await browserService.waitForCdp(25_000);
            if (cdpReady) {
              const cdp = browserService.getCdp()!;
              return await screenshotWithSnapshot(cdp, 2000, `Opened browser: ${input.url}`);
            }
            return textResult(`Opened browser: ${input.url}\nNote: CDP automation not available — screenshot and interaction tools will not work.`);
          } catch (error) {
            return errorResult('browser_open', error);
          }
        },
      ),
      ...
      tool(
        'browser_navigate',
        'Navigate the integrated browser to a new URL. The user sees navigation live in the VS Code panel. IMPORTANT: Provide the exact, complete URL path. Do not guess URL paths — look up routes in the codebase or ask the user. Wrong paths will silently load a 404 or wrong page. Returns a screenshot after navigation.',
        {
          url: z.string().describe('Exact full URL to navigate to'),
        },
        async (input) => {
          try {
            await browserService.open(input.url);
            ...
            if (cdpReady) {
              const cdp = browserService.getCdp()!;
              return await screenshotWithSnapshot(cdp, 1500, `Navigated to: ${input.url}`);
            }
            return textResult(`Navigated to: ${input.url}\nNote: CDP automation not available — screenshot not captured.`);
          } catch (error) {
            return errorResult('browser_navigate', error);
          }
        },
      ),
      ...
      tool(
        'browser_screenshot',
        'Capture a screenshot of the current page. The user sees the page live, but you need this to see it. To discover interactive elements (buttons, inputs, links, selects), prefer browser_query — it returns structured data with reliable selectors.',
        {},
        async () => {
          try {
            const cdp = requireCdp('browser_screenshot');
            const screenshot = await cdp.captureScreenshot();
            return imageResult(screenshot, `Screenshot of: ${browserService.getCurrentUrl() ?? 'current page'}`);
          } catch (error) {
            return errorResult('browser_screenshot', error);
          }
        },
      ),
      ...
      tool(
        'browser_query',
        'List all interactive elements on the page with their text, attributes, and viewport position. Elements marked offScreen:true are below the viewport fold — scroll down to discover and interact with them. Reports scroll state and empty form fields. Each element is tagged with data-dq="N" — use [data-dq="N"] as a CSS selector in other browser tools. Call this BEFORE interacting with a page. IMPORTANT: Always check the scroll/empty fields metadata before submitting forms.',
        {
          filter: z.string().optional().describe('Filter by tag name or ARIA role. Comma or space separated (e.g. "input,select", "button combobox", "option")'),
        },
        async (input) => {
          try {
            ...
            if (!elements || (Array.isArray(elements) && elements.length === 0)) {
              return textResult('No interactive elements found on the page.');
            }
            ...
            return textResult(queryLines.join('\n'));
          } catch (error) {
            return errorResult('browser_query', error);
          }
        },
      ),
      ...
      tool(
        'browser_click',
        'Click an element. Provide EITHER a CSS selector OR visible text content. Use [data-dq="N"] selectors from browser_query for maximum reliability. Standard CSS only — Playwright pseudo-selectors like :has-text() do NOT work. Returns a screenshot.',
        {
          selector: z.string().optional().describe('CSS selector (e.g. [data-dq="3"], button.submit, #login-btn)'),
          text: z.string().optional().describe('Visible text of the element to click (e.g. "Sign in", "Submit", "Objectives")'),
        },
        async (input) => {
          try {
            const cdp = requireCdp('browser_click');
            if (!input.selector && !input.text) {
              return { content: [{ type: 'text' as const, text: 'Error: Provide either selector or text parameter.' }], isError: true as const };
            }
            if (input.text) {
              const result = await cdp.evaluate(`(() => {
              ...
              if (!result.value) {
                return { content: [{ type: 'text' as const, text: `Error: No clickable element found with text "${input.text}".` }], isError: true as const };
              }
              ...
              return await screenshotAfter(cdp, 500, `Clicked: ${desc}`);
            }
            await cdp.clickSelector(input.selector!);
            return await screenshotAfter(cdp, 500, `Clicked: ${input.selector}`);
          } catch (error) {
            return errorResult('browser_click', error);
          }
        },
      ),
      ...
      tool(
        'browser_type',
        'Type text into an input field. Use [data-dq="N"] selectors from browser_query for reliable targeting. Optionally click a selector first to focus it. Set clear=true to clear existing text before typing. Returns a screenshot.',
        {
          text: z.string().describe('Text to type'),
          selector: z.string().optional().describe('CSS selector to click/focus before typing'),
          clear: z.boolean().optional().describe('Clear existing text in the field before typing'),
        },
        async (input) => {
          try {
            ...
            return await screenshotAfter(cdp, 300, `Typed "${input.text}"${input.selector ? ` into ${input.selector}` : ''}`);
          } catch (error) {
            return errorResult('browser_type', error);
          }
        },
      ),
      ...
      tool(
        'browser_evaluate',
        'Execute JavaScript in the page context and return the result. Code with return statements is auto-wrapped in an IIFE. Expressions with const/let are auto-wrapped in a block scope. Prefer browser_query + browser_click/type/select for standard interactions.',
        {
          expression: z.string().describe('JavaScript expression to evaluate (e.g. "document.title", "document.querySelectorAll(\'input\').length")'),
        },
        async (input) => {
          try {
            ...
            return errorResult('browser_evaluate', error);
          }
        },
      ),
      ...
      tool(
        'browser_element',
        'Get details about a DOM element: outer HTML, bounding box dimensions, and computed visual styles. Useful for inspecting page structure before interacting.',
        {
        ...
        async (input) => {
          try {
            ...
            if (!nodeId) return textResult(`Element not found: ${input.selector}`);
            ...
            return errorResult('browser_element', error);
          }
        },
      ),
      ...
      tool(
        'browser_console',
        'Get recent browser console messages. Useful for debugging JavaScript errors, checking application logs, or verifying API responses logged to console.',
        {},
        async () => {
          try {
            const messages = browserService.getConsoleMessages();
            if (messages.length === 0) return textResult('No console messages captured.');
            const text = messages.map(m => `[${m.level}] ${m.text}`).join('\n');
            return textResult(text);
          } catch (error) {
            return errorResult('browser_console', error);
          }
        },
      ),
      ...
      tool(
        'browser_network',
        'Get recent network errors (failed requests, HTTP 4xx/5xx responses). Useful for debugging API calls or asset loading issues.',
        {},
        async () => {
          try {
            const errors = browserService.getNetworkErrors();
            if (errors.length === 0) return textResult('No network errors captured.');
            const text = errors.map(e =>
              e.status ? `[${e.status} ${e.statusText ?? ''}] ${e.url}` : `[${e.type}] ${e.url}`,
            ).join('\n');
            return textResult(text);
          } catch (error) {
            return errorResult('browser_network', error);
          }
        },
      ),
      ...
      tool(
        ...
        'Get the accessibility tree of the page. Useful for understanding page structure and finding interactive elements when CSS selectors are unclear.',
        {},
        async () => {
          try {
            ...
            return textResult(text.length > 8000 ? text.slice(0, 8000) + '\n... (truncated)' : text);
          } catch (error) {
            return errorResult('browser_accessibility', error);
          }
        },
      ),
      ...
      tool(
        'browser_hover',
        'Move the mouse over an element to trigger hover state. Returns a screenshot. Useful for revealing tooltips, dropdown menus, or hover-dependent UI.',
        {
        ...
        async (input) => {
          try {
            ...
            return await screenshotAfter(cdp, 300, `Hovered: ${input.selector}`);
          } catch (error) {
            return errorResult('browser_hover', error);
          }
        },
      ),
      ...
      tool(
        'browser_scroll',
        'Scroll the page or a scrollable container (dialog, modal, overflow div). Auto-detects the scrollable container — works inside modals, dialogs, and overflow divs, not just the window. If neither x nor y is provided, scrolls down by ~75% of viewport height. When a selector is provided, finds the nearest scrollable element within or around it. Returns a screenshot.',
        {
          x: z.number().optional().describe('Horizontal scroll pixels (positive = right)'),
          y: z.number().optional().describe('Vertical scroll pixels (positive = down, e.g. 500)'),
          selector: z.string().optional().describe('CSS selector of a specific scrollable container (auto-detected if omitted)'),
        },
        async (input) => {
          try {
            ...
            if (r.error) return errorResult('browser_scroll', new Error(r.error));
            ...
            return await screenshotAfter(cdp, 300, parts.join(' '));
          } catch (error) {
            return errorResult('browser_scroll', error);
          }
        },
      ),
      ...
      tool(
        'browser_select',
        'Select an option from any dropdown — native <select> or custom components (Radix, Reka, Headless UI, shadcn). For native selects, sets value directly. For custom selects (combobox triggers), clicks the trigger then clicks the matching [role="option"] via CDP coordinates. Returns a screenshot.',
        {
          selector: z.string().describe('CSS selector of the select trigger (e.g. [data-dq="5"], #my-select)'),
          value: z.string().optional().describe('The value attribute or data-value of the option'),
          text: z.string().optional().describe('The visible text of the option (e.g. "High", "IT & Digital Services")'),
        },
        async (input) => {
          try {
            const cdp = requireCdp('browser_select');
            if (!input.value && !input.text) {
              return { content: [{ type: 'text' as const, text: 'Error: Provide either value or text parameter.' }], isError: true as const };
            }
            const tagResult = await cdp.evaluate(
              `document.querySelector(${JSON.stringify(input.selector)})?.tagName`,
            );
            if (!tagResult.value) {
              return errorResult('browser_select', new Error(`Element not found: ${input.selector}`));
            }
            if (tagResult.value === 'SELECT') {
              ...
              if (!optResult.value) {
                return errorResult('browser_select', new Error(
            ...
            return await screenshotAfter(cdp, 300, `Selected "${label}" in ${input.selector}`);
          } catch (error) {
            return errorResult('browser_select', error);
          }
        },
      ),
      ...
      tool(
        'browser_fill',
        'Fill multiple form fields in a single call with post-fill verification. Each text field is verified after filling — WARN is reported if the value did not stick (read-only field, wrong selector, framework rejection). Much faster than separate browser_type/browser_select calls — text is inserted in one operation (not character-by-character). Prefer this over individual calls when filling 2+ fields. Use stable CSS selectors (#id, [name="..."], or [data-dq="N"] from browser_query). Field types: "text" (default) clears and types into input/textarea. "select" handles native and custom dropdowns. "date" takes ISO YYYY-MM-DD — pass the calendar trigger button selector (e.g. the "Pick a date" button); auto-opens the popover, navigates months, and clicks the target day in one step. "check"/"uncheck" toggles checkboxes/switches. Returns a screenshot.',
        {
          fields: z.array(z.object({
            selector: z.string().describe('CSS selector of the form field (e.g. #title, [data-dq="5"], [name="priority"])'),
            value: z.string().describe('Value: text content, option text/value, ISO date YYYY-MM-DD, or ignored for check/uncheck'),
            type: z.enum(['text', 'select', 'date', 'check', 'uncheck']).optional().describe('Field type: "text" for inputs/textareas (default), "select" for dropdowns, "date" for calendar trigger buttons (YYYY-MM-DD), "check"/"uncheck" for checkboxes'),
          })).min(1).max(30).describe('Form fields to fill'),
        },
        async (input) => {
          try {
            ...
            return await screenshotAfter(cdp, 500, results.join('\n'));
          } catch (error) {
            return errorResult('browser_fill', error);
          }
        },
      ),
      ...
      tool(
        'browser_wait',
        'Wait for an element to appear on the page. Provide EITHER a CSS selector OR visible text content. Returns a screenshot once the element is found.',
        {
          selector: z.string().optional().describe('CSS selector to wait for'),
          text: z.string().optional().describe('Visible text content to wait for (e.g. "Delete", "Loading complete")'),
          ...
          try {
            ...
            if (input.text) {
              ...
              for (let i = 0; i < maxAttempts; i++) {
                ...
                if (found.value === true) {
                  const screenshot = await cdp.captureScreenshot();
                  return imageResult(screenshot, `Text appeared: "${input.text}"`);
                }
                await new Promise(r => setTimeout(r, pollInterval));
              }
              return { content: [{ type: 'text' as const, text: `Error: Timeout waiting for text: "${input.text}"` }], isError: true as const };
            }
            await cdp.waitForSelector(input.selector!, timeoutMs);
            const screenshot = await cdp.captureScreenshot();
            return imageResult(screenshot, `Element appeared: ${input.selector}`);
          } catch (error) {
            return errorResult('browser_wait', error);
          }
        },
      ),
      ...
      tool(
        'browser_drag',
        'Drag an element to a drop target using HTML5 Drag-and-Drop events (dragstart/dragover/drop). Also dispatches pointer events for frameworks that use them. Returns a screenshot showing the result — verify the drag visually.',
        {
        ...
        async (input) => {
          try {
            ...
            if (val?.error) {
              return { content: [{ type: 'text' as const, text: `Error: Element not found: ${val.error === 'source not found' ? input.sourceSelector : input.targetSelector}` }], isError: true as const };
            }

            return await screenshotAfter(cdp, 500, `Dragged ${input.sourceSelector} → ${input.targetSelector}`);
          } catch (error) {
            return errorResult('browser_drag', error);
          }
        },
      ),
      ...
      tool(
        ...
        async () => {
          try {
            ...
            return errorResult('browser_snapshot', error);
          }
        },
      ),
      ...
      tool(
        'browser_act',
        'Perform actions using [ref] numbers from the most recent page snapshot (from browser_snapshot, browser_open, browser_navigate, or a previous browser_act). Supports batching multiple actions in one call for maximum speed. Returns a fresh page snapshot after all actions complete. Actions: click (ref), type (ref + text, optional clear), select (ref + value, handles native & custom dropdowns), key (key name like Enter/Escape/Tab, optional ref to focus first), hover (ref).',
        {
          actions: z.array(z.object({
            action: z.enum(['click', 'type', 'select', 'key', 'hover']).describe('Action type'),
            ref: z.number().optional().describe('Element [ref] number from snapshot (required for click/type/select/hover)'),
            text: z.string().optional().describe('For type: text to enter. For key: key name (Enter, Escape, Tab, ArrowDown, Backspace)'),
            value: z.string().optional().describe('For select: option text or value to select'),
            clear: z.boolean().optional().describe('For type: clear field before typing'),
          })).min(1).max(20).describe('Actions to perform sequentially'),
        },
        async (input) => {
          try {
            ...
            return errorResult('browser_act', error);
          }
        },
      ),
      ...
      tool(
        'browser_close',
        'Close the browser and end the automation session. Closes the VS Code browser panel, stops the headless Chrome process, and releases all resources.',
        {},
        async () => {
          try {
            ...
            return errorResult('browser_close', error);
          }
        },
      ),
    ],
