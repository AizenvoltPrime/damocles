...
export const DEFAULT_AGENTS: Map<string, AgentConfig> = new Map([
  [
    'general-purpose',
    {
      ...
      // builtinToolNames omitted, meaning "all available tools" (resolved at lookup time).
      extensions: true,
      skills: true,
  ...
  [
    'Explore',
    {
      ...
      displayName: 'Explore',
      description:
        'Fast read-only search agent for locating code and researching the web. Use it to find files by pattern (eg. "src/components/**/*.tsx"), grep for symbols or keywords (eg. "API endpoints"), answer "where is X defined / which files reference Y," or research online sources (library docs, releases, public source) via its read-only web tools when web access is enabled. It can also inspect a live page or a running app with the integrated browser when that is enabled, so use it for "what does this URL actually render / log / request" questions. Do NOT use it for code review, design-doc auditing, cross-file consistency checks, or open-ended analysis, because it reads excerpts rather than whole files and will miss content past its read window. When calling, specify search breadth: "quick" for a single targeted lookup, "medium" for moderate exploration, or "very thorough" to search across multiple locations and naming conventions.',
...
# Design Discipline
- Design to industry standards, with no bandaids and no cut corners. Plan the correct, durable solution, never the expedient one. Apply the established best practices for the domain (OWASP for security, REST/GraphQL conventions for APIs, SOLID for OOP, 12-factor for services, idiomatic patterns for the language/framework) unless this codebase already commits to a different approach. When you depart from a norm, say so and justify it.
