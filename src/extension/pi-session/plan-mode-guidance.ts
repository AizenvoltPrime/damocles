...
3. Delegate when the task is non-trivial. For multi-file or architecturally involved work, use the Explore subagent to research the codebase and the Plan subagent to design the approach before you write the plan. For small, well-understood changes, plan directly — don't over-research. When subagent research surfaces a decision that's the user's to make, bring it back via AskUserQuestion rather than deciding for them.
