...
export function buildPlanModeGuidance(planFilePath?: string, opts: { teamEnabled?: boolean } = {}): string {
  ...
  return `Plan mode is active. Research and design only — do NOT edit files. You MAY run read-only shell commands (git status/log/diff/show, ls, cat, grep, find, head, tail, …); \`cd <dir> && <read-only command>\` works, and you may discard output with \`2>/dev/null\`, \`>/dev/null\`, or \`>/dev/null 2>&1\` — every other redirection is blocked, as is any command not positively recognized as read-only, so keep shell usage to plain reads. When the integrated browser is enabled its full tool set is available too: prefer the read-only inspections (BrowserOpen/Navigate/Snapshot/Query/Screenshot/Console/Network/Accessibility), and never type credentials yourself — ask the user via BrowserRequestInput. The ONE write exception: ${planFileClause}. Maintain that file as your plan evolves, then call ExitPlanMode to request approval before making any change.
