...
export function buildPlanModeGuidance(planFilePath?: string, opts: { teamEnabled?: boolean } = {}): string {
  ...
  return `Plan mode is active. Research and design only — do NOT edit files. You MAY run read-only shell commands (git status/log/diff/show, ls, cat, grep, find, head, tail, …); any command not positively recognized as read-only is blocked, so keep shell usage to plain reads. The ONE write exception: ${planFileClause}. Maintain that file as your plan evolves, then call ExitPlanMode to request approval before making any change.
