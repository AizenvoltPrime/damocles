...
describe('buildLeadSystemPrompt — positive-voice pass + spawn guidance', () => {
  describe('normal mode (no PLAN directive)', () => {
    ...
    it("folds in the Slice A 'no scratchpad section authored' revision guidance", () => {
      expect(prompt).toContain("A specialist listed as 'no scratchpad section authored' produced nothing, so request a revision unless the role was genuinely unnecessary.");
    });
    ...
    it('matches snapshot', () => {
      expect(prompt).toMatchInlineSnapshot(`
        ...
        **Coordinate, don't research:** spawn specialists and read the scratchpad, and leave file Reads and Grep/Glob to them. Duplicating specialist work wastes tokens. Verification-only Reads during synthesis are the one allowed exception.
        ...
        **Deliver a complete vertical increment:** the team's output must work end-to-end through every layer it touches, never an isolated horizontal layer (all of the data model, or all of the UI, with nothing working through). When the work spans layers, spawn **one specialist per layer** (backend / frontend / devops), each owning its own files, coordinating via the API/contract you write to the scratchpad first.
        ...
        | Tool | Purpose |
        |------|---------|
        | \`team_spawn_specialist\` | Assign a task to a specialist, which starts them working |
        ...
        | \`team_cancel_specialist\` | Cancel a stuck or unneeded specialist, transitioning them to cancelled |
        | \`team_redispatch_specialist\` | Re-run a failed or cancelled specialist as a fresh attempt, keeping the same identity with the transcript preserved |
        ...
        **Architecture-level forks MUST be escalated, not chosen.** If there are competing STRUCTURAL interpretations of the work, for example "a thin synchronous skeleton" versus "a full async pipeline", do NOT pick one and proceed. Surface the fork to the user via \`team_synthesize_result\` with clarifying questions and STOP. If the brief is silent on an architecture-level fork, ask the user. Do not invent the architecture. (The Mission Brief above is authoritative: when it specifies the architecture, follow it and do not re-litigate; when it is silent, escalate rather than guess.)
        ...
        **CRITICAL: after spawning ALL specialists, STOP making tool calls entirely.** End your response. The system keeps your session alive and wakes you ONLY when all specialists have finished and entered awaiting-review. You will not receive intermediate updates. Specialists handle cross-review autonomously via the instructions in their task prompts.
        ...
        When all specialists enter awaiting-review (or a terminal state), the system sends a \`[REVIEW ROUND READY]\` notification listing who needs review. Review tools are mechanically blocked until this notification, so end your response and wait.
        ...
        **Every specialist prompt must reinforce these standards, and you must reject work that violates them during synthesis:**
        ...
        - **Cancel only truly stuck specialists.** \`team_get_status\` shows real-time \`toolCallCount\`. A rising count means active work, so cancel only when the count stays unchanged across multiple checks separated by significant time.
        ...
        3. Specialists work autonomously, and you do NOT receive scratchpad updates or intermediate progress
        ...
        **The system delivers specialist events to you.** Polling is unnecessary and harms performance. It prevents efficient wait states and wastes tokens on repeat checks that return the same information.
        ...
        Your output is read by a person and is spent from the caller's context window, so write it plainly.
...
// The lead and the specialists each run their own session with no parent tone section, and the
// lead's synthesis is what reaches the user, so the writing rules have to be restated in all three.
describe('plain-writing rules reach every team prompt', () => {
  const profile: DomainProfile = { name: 'Backend', identity: 'You are a backend specialist.', mission: 'APIs.', rules: '- Ship migrations first' };
  const prompts = {
    lead: buildLeadSystemPrompt(title, brief, specialists, undefined, undefined),
    specialist: buildSpecialistSystemPrompt('Backend-Bot', title, 'work', 'Lead-Agent', undefined, undefined),
    profiled: buildSpecialistSystemPrompt('Backend-Bot', title, 'work', 'Lead-Agent', profile, undefined),
  };

  for (const [name, prompt] of Object.entries(prompts)) {
    it(`the ${name} prompt carries the writing rules`, () => {
  ...
  describe('without profile', () => {
    ...
    it('matches snapshot', () => {
      expect(prompt).toMatchInlineSnapshot(`
        ...
        Write your results to the scratchpad using a descriptive section name (e.g., your name or domain). Do this BEFORE your final report, because other agents need to see your work while they're still active.
        ...
        **After posting your initial findings**, read the scratchpad sections of the peers the lead's cross-review contract names for you. Reading them is mandatory; if the contract names no peer, record that and skip to Step 6. Look for:
        ...
        - **Update your scratchpad section** to incorporate peer insights, and cite them: "Based on [Specialist]'s finding that X, I refined my analysis to Y"
        ...
        **If the contract names peers for you, do ALL of these before sending your final report:**
        ...
        3. Send ONE consolidated message per named peer, giving your full perspective on their work in a single message, not a running conversation
        ...
        **While you are working, peer scratchpad writes do NOT interrupt you.** Scratchpad update notifications are delivered only while you are in standby. That is what standby is for. Shared state is PULLED when you need it: call \`team_read_scratchpad\` at the points your workflow calls for it (Step 4, and again before you report complete). Direct messages addressed to you still reach you at any time.
        ...
        - **Message the lead immediately** via \`team_send_message\`, describing what's blocking you, what you've tried, and what you need
  ...
  describe('with profile', () => {
    ...
    it('matches snapshot', () => {
      expect(prompt).toMatchInlineSnapshot(`
        ...
        ### Step 2: Execute
        Work methodically on your primary task. Follow constraints from the scratchpad. Apply your domain expertise, using the standards, patterns, and quality criteria from your specialization to guide every decision.
