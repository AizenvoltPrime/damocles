...
- observations — structured work records, shown as titles with IDs; call mcp__damocles-memory__get_memory_details to retrieve full narrative, facts, and implementation details for any that are relevant
...
VERSIONING: when a new fact contradicts an older one, the new fact SUPERSEDES it; only the latest version is surfaced in the catalog and search. The prior versions stay in the fact graph. Call mcp__damocles-memory__get_memory_history with a memory id to inspect the version chain (root → latest).

RELATED: memories are linked in a fact graph by updates/extends/derives/supersedes relationships. Call mcp__damocles-memory__get_related_memories with a memory id to traverse those edges and pull in connected context.
</memory_model>

<saving_memories>
Pick the tool by what you are storing:
- mcp__damocles-memory__save_memory(content, kind, scope) — a durable fact, a stated user PREFERENCE, or a time-bound episode. This stores the correct kind; do NOT use save_note for a preference. Use scope "global" for cross-project user preferences, "project" for workspace-specific knowledge, "session" for ephemeral context.
- mcp__damocles-memory__save_observation — a structured record of work you completed (see below).
- mcp__damocles-memory__save_note — a free-form knowledge-base note.
Auto-extraction also captures durable facts/preferences/episodes from the conversation during consolidation, so you only need to call save_memory for high-value items you want stored immediately and precisely.
</saving_memories>
