...
- **`~/.damocles/AGENTS.md` (or `CLAUDE.md`) is the user-global instructions file.** The global instructions slot resolved to pi's agent directory, which Damocles pins to `~/.damocles/pi/agent/`. That is pi's internal state directory, not the place beside `settings.json`, `hooks.json` and `mcp.json` where you keep everything else, so in practice `~/.claude/CLAUDE.md` was the only findable way to give the agent instructions that apply in every workspace. The top level of `~/.damocles/` is now searched for pi's own candidate names, and the first match takes the global slot in every workspace and appears in `/context`. No new filename is introduced and `/init` still writes `CLAUDE.md`. Only the global slot moved: project discovery, including the parent-directory walk and the linked-worktree shadowing rule, is unchanged, and with no file at the top level of `~/.damocles/` the old `~/.damocles/pi/agent/AGENTS.md` still loads.

- **`.damocles/skills/` and `.damocles/commands/` are read, and they outrank `.claude` and `.codex`.** Damocles already kept its permission settings in `.damocles/` and read them ahead of the Claude Code equivalents in every tier. Skills and slash commands were the exception: they only ever came from `.claude` and `.codex`, so a project that wanted a Damocles-specific skill had to write it into a directory named after a different tool. Both directory families are now scanned, project scope before user scope within each source, and `.damocles` wins any name collision. It is deliberately outside `damocles.assetSourcePrecedence`, which stays a tie-break between `.claude` and `.codex` only, so there is one rule to remember rather than a new setting. The directories are watched like the existing ones, so creating `<workspace>/.damocles/skills/foo/SKILL.md` puts `/foo` in the menu and in the agent's loaded set within about 300 ms, with no window reload. pi's own `.pi` directories still rank above all of these; that is unchanged.

- **Servers added with a plain `claude mcp add` now show up.** Damocles read only the top-level `mcpServers` of `~/.claude.json`, which is Claude Code's _user_ scope. `claude mcp add` defaults to its **local** scope, which lands in `projects[<workspace>].mcpServers` in that same file, so the most common way to add a server to Claude Code produced nothing in the Damocles panel. That scope is now read for the open workspace and badged as coming from Claude Code, ranked above the user scope to match Claude Code's own local over project over user ordering. It is never withheld in an untrusted workspace: the file lives in your home directory, so a repository you cloned cannot have written it.

- **`<workspace>/.damocles/mcp.local.json` is a personal per-project MCP file.** There was no way to add an MCP server for one project without either committing it to the project's `.mcp.json` or making it global in `~/.damocles/mcp.json`. This file is read for the open workspace and outranks every other source, so it is also how you override a server the team's `.mcp.json` defines. It is deliberately personal rather than a second committed file: silently outranking `.mcp.json` would leave a teammate reading the wrong config with no signal. It carries the `.local` suffix the settings files already use, is gitignored by this repository, has no write path in the panel, and is watched, so editing it re-merges and reconnects within about 300 ms with no window reload. It is trust-gated exactly like `.mcp.json`, because a hostile repository could ship one.

- **A Reload config action in the MCP panel.** `~/.claude.json` is not watched, because Claude Code rewrites it continuously and each event would re-drive the live client and cycle every connection. That left a server added there invisible until a window reload, which the Claude Code local scope now makes the common case. The button re-reads every source, re-feeds the live client and refreshes every open panel.

- **A warning when `.damocles/mcp.local.json` is not gitignored.** The file is the intended home for a personal MCP definition, `env` and `headers` values included, and it sits in the working tree, so in your own repository it is one `git add` away from publishing a credential. When the file exists and git is not ignoring it, or it is already tracked despite a later ignore rule, the panel says so and names the line to add. Damocles never edits your `.gitignore`: the check asks git in your repository, not the private checkpoint repository, and a workspace that is not a git repository raises nothing. `.gitignore` is watched alongside the MCP config files, so adding the line clears the warning within about 300 ms instead of leaving it up until the next config edit. The check is skipped in a workspace you have not trusted, because git reads that repository's own `.git/config` and `core.fsmonitor` there is a command git runs, and the servers the warning protects are withheld in that state anyway. Granting trust re-reads every source, so the warning appears then rather than waiting for the next config edit. It gives up after ten seconds, so a virtualised filesystem or a hung fsmonitor helper cannot stall session start with no way out but a window reload.

- **`.damocles/agents/` is read, and it outranks both `.claude/agents` and `.pi/agents`.** Custom agents came only from the `.claude` and `.pi` directories, so a project that wanted a Damocles-specific agent had to write it into a directory named after a different tool. `<workspace>/.damocles/agents/*.md` and `~/.damocles/agents/*.md` are now scanned as well, and `.damocles` wins a name collision with the other two at the same scope. Agents are the one asset where `.damocles` ranks above `.pi`, because Damocles owns the agent discovery order outright instead of handing the load to pi as it does for skills and prompts. The new directories get the same watcher as the existing ones, so creating `<workspace>/.damocles/agents/reviewer.md` puts `@agent-Reviewer` in the `@` menu within about 300 ms with no window reload. The project directory is trust-gated like `.claude/agents` and `.pi/agents`, so it loads nothing until you trust the workspace. `/context` and the `@` menu list a `.damocles` project agent with scope `project`.

### Changed

- **Behavior change for existing `.claude` and `.codex` users: project skills and commands are withheld from an untrusted workspace.** A `SKILL.md` is text that goes into the system prompt, so opening an unfamiliar repository in Restricted Mode used to be enough for that repository to steer the agent. Project-scope skills and commands from every source, `.claude` and `.codex` included, are now kept out of the agent's loaded set until you trust the workspace. They stay visible in the `/` menu with an untrusted badge rather than vanishing, because a silent disappearance reads as breakage, and selecting one shows a message naming workspace trust instead of starting a turn the agent cannot service. User-scope skills and commands, the ones under `~/`, are unaffected. Granting trust loads the project ones immediately with no window reload. If you keep project skills in a workspace you have never explicitly trusted, trust it once and they come back.

- **Adding a server from the panel is rejected when a higher-precedence file already defines that name, and the rejection names the file.** The check covered the project's `.mcp.json` only and said so in the message. It is now derived from the merge order itself, so `.damocles/mcp.local.json` is covered too and the two cannot drift apart, and the inline error names whichever file owns the name. It reads the order the merge actually used, so in a workspace you have not trusted, where the two repository-authored files are folded below your own config rather than above it, the name is accepted instead of being refused on a precedence that does not apply. Granting trust restores the rejection in the same re-read.

- **Skill approval prompts show the description for `.codex` and `.damocles` skills.** The reader behind the approval prompt looked only in `.claude/skills/`, so a `.codex` skill was approved with no description at all even though the menu listed one. It now walks the same source list as everything else.

- **A rewind restores agent edits under `.damocles/`.** The checkpoint system excluded the whole directory. That was correct when it held nothing but settings, and it would now mean a skill or command the agent wrote during a turn survives a rewind of that turn. Three files stay excluded. `.damocles/settings.json` and `.damocles/settings.local.json` stay out because the permission and MCP panels write them outside the turn, and a rewind must not revoke a permission you granted mid-turn. `.damocles/mcp.local.json` stays out for a different reason: no panel writes it, and it is a gitignored personal file holding `env` and `headers` credentials, which must never enter the checkpoint repository. This applies to shadow repositories created from this version on. An existing one keeps the old blanket exclude, permanently, because its earlier checkpoints hold no `.damocles` content and a rewind to one of them would delete the whole directory. The cost is that a project you have already used never gains undo for agent-edited skills.

- **The `~/` skill and command directories are watched for real.** A user-scope watcher was registered with a plain string glob pointing at your home directory, and VS Code reports no event from outside the open workspace folders for that kind of pattern, so none of those watchers ever fired. Creating `~/.damocles/commands/foo.md` left the menu unchanged until something else invalidated the cache or you reloaded the window. Each one is now anchored on the directory's own URI, the form the agent-directory watcher already used, so all three sources refresh within about 300 ms at user scope exactly as they do at project scope.

- **A withheld project skill no longer disables a working command of the same name.** In a workspace you had not trusted, an untrusted project skill claimed the name before anything checked for a command, so a repository that shipped `.damocles/skills/commit/SKILL.md` could stop your own `~/.damocles/commands/commit.md` from running just by being opened. Both lookups now happen before either can answer, a runnable asset of either kind wins, and the trust message appears only when everything claiming the name is withheld.

- **A command or skill whose name contains a dot or a namespace is handled like any other.** The scanner accepted `foo.bar.md` and listed `/foo.bar`, and the subdirectory scan listed `/ns:name`, but the chat input recognised neither as an asset invocation. An untrusted one got no refusal, a trusted skill was never expanded, and the approval prompt showed no description. One alphabet now defines what a name may contain, and the scanner, the chat input and the approval reader all use it.

- **The slash menu no longer reads through a symlink.** A link under a commands directory, or in place of a `SKILL.md`, was followed and the first line of the file behind it became a menu description, in an untrusted workspace included. Links are now skipped in both positions, matching what the agent loader and the custom-agent scanner already did.

- **A skill's `name:` frontmatter decides its name in the menu.** The agent loads a skill under its frontmatter name and the menu listed it under its directory name, so a skill that set both was pre-approved under one name and invoked under the other. The menu now uses the frontmatter name when there is one, and skips a skill whose declared name is not a valid asset name.

- **Changing `damocles.assetSourcePrecedence` takes effect immediately.** Both scans cache their results, and nothing cleared the caches when the setting changed, so the previous winner of a contested name kept its place until a watched file changed or you granted trust. The setting is now watched and both caches are dropped when it changes.

- **A name collides case-insensitively, as the filesystem does.** `.damocles/skills/Foo` and `.claude/skills/foo` produced two rows on Windows and macOS where the filesystem holds one identity, and `/Foo` against a withheld `foo` produced no trust message. Names are matched without regard to case throughout, and each entry keeps the case it was written in.

- **A built-in command is no longer shadowed by a menu row that does nothing.** A project `.damocles/commands/compact.md` added a second `/compact` row, and picking either ran the built-in, so in an untrusted workspace the extra row carried a trust badge for something that ran regardless. A custom command or skill named after a built-in is dropped from the menu.

- **With no folder open, your `~/` assets are user-scope.** The panel passed your home directory in place of the workspace path, so every user directory was scanned twice and the first pass won, leaving `~/.damocles/skills/foo` badged as a project skill. Asset discovery is now told there is no project scope, while session storage and history keep using the home directory as before.

- **A subagent's `skills:` field no longer reads an untrusted workspace.** A `skills:` entry on one of your own global agents was resolved against `<cwd>/.pi/skills` and `<cwd>/.agents/skills` with no trust check, so opening an unfamiliar repository and running that agent spliced the repository's skill text into the agent's system prompt. The two project roots are skipped until you trust the workspace. The three user roots are unaffected.

- **Behavior change for existing Compass users: Compass does not index a workspace you have not trusted.** Compass walks the whole workspace tree, parses every source file into its graph, hands the model raw source lines through `compass_review_context`, and runs `git diff` in the repository to find changed files. Git reads that repository's own `.git/config` before anything else, and `core.fsmonitor` there is a command git runs, so opening an unfamiliar clone with Compass on was enough to execute something the repository shipped. Listing `damocles.compass.enabled` as a restricted configuration only stopped a repository turning Compass on for itself, which is not the common case: anyone who uses Compass has enabled it for every workspace. Compass now stays off until you trust the workspace, with no index built, no file watcher, no sidebar views, no Compass tools offered to the agent and no Compass section in the system prompt. Granting trust starts it immediately with no window reload. Every git call Compass makes now also passes `-c core.fsmonitor=false` and an argument list instead of a shell command string.

- **Two agent files in one directory declaring the same name resolve the same way everywhere.** The directory scan took whatever order the filesystem returned and the last file loaded won, so which agent you got depended on the machine. Entries are visited in name order, so the alphabetically last file wins.
...
- **MCP servers are now discovered in `~/.damocles/mcp.json` and `~/.codex/config.toml`, not just Claude's files.** Damocles has a user-global MCP file of its own for the first time and reads Codex's config alongside it, so a server you already set up for either tool appears in the panel labelled with where it came from, without being copied anywhere first. Four sources merge, a later one winning a name collision: the Claude and Codex imports, then `~/.damocles/mcp.json`, then the project's `.mcp.json`. `damocles.assetSourcePrecedence` breaks the Claude-versus-Codex tie — the same setting that already decides whose skills and slash commands win. A server marked `enabled = false` in Codex is not started here either, because turning it off there was a decision. From a Codex entry the command, arguments, environment and URL come across, and `startup_timeout_sec` / `tool_timeout_sec` are left behind. Both `env_vars` and `bearer_token_env_var` stay variable _names_, resolved when the server starts, so a Codex `OPENAI_API_KEY` is never copied into memory. Hand-editing any of these files refreshes the panel with no window reload.

  **Switching a server off is remembered per project.** A server defined globally still appears everywhere, but turning it off applies to the project you turned it off in — so a server that is useful in one repository and noise in another can be on in the first and off in the second, without maintaining two config files.

- **MCP servers can now be added, edited and removed from the panel.** Defining one used to mean finding the right file and writing JSON by hand. There is now an **Add server** button and, on the rows Damocles owns, **Edit** and **Delete**. The form covers both kinds of server — a command with its arguments, environment and working directory, or a URL with its transport and headers — and validates what you typed before anything reaches disk, so a definition that would not have worked is refused with the reason rather than left to fail at connect time. Deleting asks first.

  **Only `~/.damocles/mcp.json` is ever written.** The Claude and Codex imports belong to those tools and your project's `.mcp.json` belongs to the project: all three are read-only here, their rows carrying no Edit or Delete button at all rather than a disabled one. Reusing a name Claude or Codex already uses is the intended way to override an import; reusing one from your project's `.mcp.json` is refused, because that file has the last word and your server would never be used. Everything else in the file survives a save — unrelated top-level keys such as `$schema`, and servers you did not touch — and two saves in quick succession cannot lose each other's change. A server you hand-wrote using options the form cannot show, such as a lifecycle or an OAuth block, can still be deleted but not edited, and the row says so.

  **A config file that cannot be read is now reported rather than silently emptied.** A stray comma used to make every server it defines vanish from the panel with no explanation anywhere. The panel now names the file and the line and offers to open it at the fault; a file that cannot be read at all is reported the same way. Only the location is shown — the parser's own message quotes the text it choked on, which may be the line holding a credential.

  **The form waits for the save to land before it closes.** It used to close the moment you pressed Save, so any refusal it could not predict — an unparseable file, a name only present in the raw file, a permission error — arrived as a notification over an empty screen with everything you had typed already gone. It now stays open until Damocles confirms the write; on a refusal it keeps your definition and shows the reason against it. Dismissing a form you have filled in asks first, and while the config file is unparseable the Add, Edit and Delete buttons are withheld rather than leading you into a form that could not have been saved.

  A remote server takes the **name of an environment variable** holding its bearer token, never the token itself. Environment and header _values_ are stored as you type them, though, and they are the ordinary home for an MCP token — so they are masked in the form behind a reveal button, the file is created readable only by you, and the form says plainly that they are stored in plain text.

### Changed

- **Permission rules you approve now land in `.damocles`, and Damocles stops writing to Claude Code's files.** Choosing "always allow" used to append the rule to `<workspace>/.claude/settings.local.json` — another tool's configuration file, edited on your behalf. It now writes `<workspace>/.damocles/settings.local.json` and touches no `.claude` file. Nothing is migrated: every rule already sitting in a `.claude` file keeps being read and keeps working, so a project set up for Claude Code behaves here the way it always has. Eight files are consulted (the README lists them), most specific first: personal overrides beat the project's shared rules, which beat your global ones, and within each level a `.damocles` rule wins over a `.claude` one — rather than a deny written for Claude Code binding Damocles with no way to override it. The first file holding a matching rule decides, checking `deny` before `allow` before `ask`; a file holding no rules is skipped rather than counting as the decision for its level. The format is byte-identical to Claude's, so these files stay readable by both tools. The destination picker now names the file it actually writes, and no longer claims the rule is gitignored — add `.damocles/settings.local.json` to your `.gitignore` yourself if you do not want your personal overrides committed.

- **A failure to save a permission rule no longer hangs the turn.** If writing the rule failed — a read-only workspace, or a settings file that was not valid JSON — the approval itself was never settled and the agent waited on that tool call indefinitely. Saving the rule is now best-effort: the approval always goes through, and a failure is logged. A settings file that does not parse is reported in the output channel rather than read as "no rules", which had let a `deny` silently disappear.

- **An untrusted project can no longer switch off one of your own MCP servers.** A `.mcp.json` naming a server you also define globally used to replace yours, and the replacement was then withheld for coming from an untrusted workspace — so opening an unfamiliar repository could silently stop your own server connecting, with the panel appearing to blame your config. A project you have not trusted no longer overrides your global servers; its own still show, still marked untrusted, and still do not run.

- **Renaming an MCP server now carries its on/off state and its disabled tools with it.** A server you had switched off came back on under its new name — which for a local server means starting a process you had deliberately stopped — and any individual tools you had turned off quietly turned back on. Deleting a server now also forgets that it was switched off, so adding the same name back later does not arrive mysteriously disabled.

- **MCP tool names may change if two of your servers have similar names.** Server names become part of every tool name with non-alphanumerics collapsed to `_`, so `my.server` and `my-server` both want `my_server` and one gets a numbered suffix. Which one used to depend on config merge order; it is now decided by sorting, so it is stable. A very long name is also shortened, because providers enforce the tool-name length limit by rejecting the whole request rather than just that tool. If you have two similarly-named servers, a tool you had switched off individually may come back on once under its new name.

- **pi runtime upgraded `0.84.1` → `0.84.2`.** All four pi packages (`pi-coding-agent`, `pi-ai`, `pi-agent-core`, `pi-tui`) move to `^0.84.2`, and loading a deferred tool group no longer throws away the prompt cache. Since 2.17.0 a `ToolSearch` activation added its tools to the top-level list, which sits inside the cached prefix — so the whole conversation was re-read at full price on the next request, and the later you loaded a group the more it cost. The activated names now travel with the result of the call that asked for them, leaving the prefix untouched. Decided per model: on the ChatGPT subscription path GPT-5.6 Sol, Terra and Luna, and with an OpenAI API key GPT-5.4, 5.4 mini, 5.4 pro and 5.5. Anthropic models advertise no equivalent and still pay the cache miss described in 2.17.0.

  The same upgrade drops the Mistral SDK for a transport pi implements itself, taking about 25 MB out of an installed Damocles and about 0.9 MB out of the VSIX, along with the duplicate `zod` 3 and `@opentelemetry/semantic-conventions` copies it pulled in — every Mistral model stays supported and reachable exactly as before. `openai` moves 6.26.0 → 6.40.0 in the same step. Separately, a model that sends `null` for an optional tool argument no longer has the call rejected: `null` is now read as the argument having been left out, which is what the model meant by it, rather than costing a retry to discover.

- **The system prompt is retuned for Claude Opus 5.** Its tone rules used to sit thousands of tokens from the end of the prompt — the memory instructions, the plan-mode guidance, your `CLAUDE.md` and your skills all land after them — so a short restatement of them now closes it. The comment policy no longer asks for comments that match the surrounding file's density: one is written only where the WHY is non-obvious, and kept to the length its point needs, however heavily commented the file already is. A correction to something said earlier is called out only when the error would change your code or your conclusions. The Explore and Plan subagents report into the main agent's context window, so both are now told to size the report against that cost, and the final summary a team submits is no longer asked for "comprehensively".

  **With thinking switched off, a tool call no longer arrives as text.** Running with `damocles.thinkingDisabled` occasionally produced a tool call written out as prose instead of actually made — it never ran, and it stayed in the conversation to mislead later turns — or an internal XML tag left in the visible answer. Guidance covering both is now sent while thinking is off, and only then. Switching thinking off is still the costlier trade: thinking on at low effort generally does better for the same money.
...
- **Subagents and team agents get your MCP tools.** They never had them: `Agent`-tool subagents stripped every `mcp__*` name, and team agents passed those names into a registry with no matching definitions, where pi dropped them with no error, no warning and no log — a specialist's tool list claimed a capability it did not have. Both now receive the panel's full eligible set, **deferred** like the panel's, so a spawn costs nothing until the agent loads a group with its own `ToolSearch`. The grant is **uniform** — `Explore`'s fixed `tools:` list, a `tools: *` agent and a team specialist all get the same set — because MCP names depend on which servers you configured, so no template can name them without hardcoding something your config breaks. `disallowed_tools` is the opt-out (exact, case-sensitive). An agent's set is **frozen at spawn**: a server connecting mid-run reaches the next agent rather than mutating a running one, and a tool its server stops advertising is reported gone rather than worth retrying.
- **A nested agent's MCP prompt renders in the panel that spawned it**, labelled with the agent that asked. Concurrent agents queue one at a time instead of overwriting each other, and an agent that ends withdraws its own prompt. With two calls in flight on one server the dialog names only the server — MCP gives no way to correlate which call elicited, and a confidently wrong name is worse than none.

### Fixed

- **A nested MCP elicitation was silently answered "cancelled".** A nested session never binds the panel's UI, and pi hands every tool a no-op UI object rather than nothing — which is truthy, so the check passed, the no-op went to the server, and its form resolved to a cancel. You never refused; the model was told nothing. Damocles now asks pi whether a UI is actually bound.
- **Disabling an MCP server no longer claims a working tool is dead forever.** Wording written for a nested agent's frozen set was reaching the main panel, where reconnecting the server brings the tool straight back.
- **Third-party MCP text can no longer forge a line in the `ToolSearch` result.** The menu was hardened against a tool name embedding a newline; the result it prints back was not, and loading a whole group is enough to reach it. A name that cannot be printed as written is now listed as omitted rather than shown in an altered form the model could not call, and stays loadable by server group.
- **A model-chosen agent name can no longer impersonate the panel.** The attribution on a nested dialog sat alone above the title with nothing marking it as a name, so a specialist called "Verified — approved" read as the panel saying so. It is now labelled, and bidi-override characters — which reverse how text renders and which no control-character filter catches — are stripped.
- **A reloaded webview no longer strands an agent.** Recreating the webview empties its dialog queue while this side still awaits answers that can never arrive, blocking that agent's tool call with nothing on screen to explain it.

### Changed

- **`Explore` and `Plan` stop naming which tool groups they can load.** "The web and browser tools" goes stale every time a group is added — it already cost five simultaneous prompt edits once. They now point at the mechanism; `ToolSearch`'s own description stays the single live inventory.
...
- **Shared verification ledger (`team_record_verification`).** Agents re-ran the full suite constantly because nothing told them a teammate had already run it — 43 full-suite runs in one measured session, with the reviewer role alone taking 46% of the spend. Each recorded run is stamped with a fingerprint of the working tree that **the extension computes from git**, not the agent: the tool has no `fingerprint` parameter and cannot be told one. A peer seeing a full-suite pass against the current fingerprint can reuse it; the moment anyone edits a file the fingerprint changes, so a pass recorded before an edit can never satisfy a check after it. The ledger is append-only and shared — no agent can overwrite it, including one that names itself `system` — and a run narrowed by a path, a name filter (`-t`), or a named script is recorded as `scoped` rather than `full-suite`, so a filtered run is never mistaken for whole-suite evidence.

### Changed

- **A scratchpad write no longer interrupts a working agent.** Every write broadcast a notice to every agent, and each notice re-prompted that agent's entire conversation. Notices now carry a structural kind and reach only an agent in `standby`, whose whole purpose is to wake on peer content; a working agent pulls shared state with `team_read_scratchpad` when it needs it. Ledger appends reach **no** inbox at all — the ledger is a pull surface, and a peer's test run cannot advance an agent parked on someone's findings. Persistence, the Timeline tab and `team_read_messages` are unchanged, so the audit trail still shows every write. The delivery decision reads the message kind rather than its text, so a peer message mimicking the notice prefix cannot spoof it.
- **Cross-review follows the lead's contract.** Every specialist was told to read every peer's section and message them, which manufactured commentary between agents whose work never touched. The lead now names which specialists actually interact; a specialist with no named peer records "no interacting layer per contract" instead of inventing feedback, and those with one send a single consolidated message rather than a thread.

### Fixed

- **The lead was re-prompted with duplicate review notifications.** `[REVIEW ROUND READY]` fired from two paths with no deduplication, landing twice a millisecond apart in 4 of 5 measured teams — each one resending a conversation that could be hundreds of thousands of tokens. Identical text is now suppressed until the underlying state actually changes. The stall nudge is deliberately exempt, since re-firing identical text is exactly how it breaks a stalled lead.
- **The verification fingerprint could go blind to real edits.** Three separate ways, each failing silently rather than reporting `unverifiable`: an untracked directory collapses to one line in `git status`, so edits inside a newly created directory changed nothing; porcelain paths are relative to the repository root, so any workspace opened at a subdirectory resolved every path to a nonexistent file; and paths containing non-ASCII or spaces come back C-quoted, naming no real file. A fingerprint that cannot see an edit makes a stale pass look reusable forever — the exact unsoundness the ledger exists to prevent. A dirty set too large to hash completely now records `unverifiable` instead of a quietly partial hash.
- **The verification ledger grew without bound.** Every append re-persisted and re-emitted the whole ledger, so cost was quadratic in the number of recorded runs and the entire history was re-injected into the agent's context on every record. The ledger is now a bounded ring, and a record call echoes only the recent entries with a count of what it elided.
- **`failures` on a passing run was silently discarded.** Recording a `pass` with a failure summary now fails with an explicit message instead of dropping the field.
...
- **`BrowserIntercept`'s blanket-pattern guard was bypassable.** The guard inspected the pattern text, so brace-group spellings (`{**}`, `{**,*}`) slipped through and could block or stub _every_ request in the shared browser, including your own tabs — while the tool description stated as fact that blanket patterns were rejected. Patterns are now compiled and tested against probe URLs, so the check matches what will actually route.
...
- **Automatic tool blocks no longer claim you rejected them.** A tool stopped by a permission rule, plan mode, a read-only agent's toolset, or a configured hook was reported to the model as _"The user doesn't want to proceed with this tool use"_ — even though you were never asked. Those blocks now say they were automatic and name the actual constraint, so the agent adapts to it instead of stopping to consult you. Genuine approval-prompt rejections are unchanged, and both still render as **denied** in the transcript.
...
- **`updateConfigAtEffectiveScope` now respects WorkspaceFolder precedence**: the helper previously only inspected `workspaceValue` / `globalValue`, ignoring `workspaceFolderValue` — VS Code's _highest_-precedence read scope. A folder-scoped setting (the default scope for `damocles.*` keys, none of which declare `scope` in `package.json`) was silently promoted to User scope on the next write, where the folder value still won at reads, so the UI toggle had no visible effect. New logic: when there's exactly one workspace folder _and_ `workspaceFolderValue` is defined, write back to `WorkspaceFolder`; otherwise fall back to the existing Workspace → Global precedence. Single-root is gated explicitly because `config.update(..., WorkspaceFolder)` throws in multi-root without a resource scope, and no current call site has a natural resource to pass. Fixes the behavior for every one of the 31 callers, not just Explore (`src/extension/chat-panel/settings-manager/utils.ts`)
