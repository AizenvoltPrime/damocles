...
| `damocles.pi.webSearch.enabled` | Enable native key-free web tools (WebSearch, WebFetch, CodeSearch) via Exa; toggling takes effect next turn, no install or reload (pi harness) | `false` |
...
The [pi](https://github.com/earendil-works/pi) agent harness — now the **default** engine for new sessions (the bundled Claude Code SDK remains only as a fallback for hosts on Node < 22) — drives a **Claude Authentication** settings panel with three modes:
