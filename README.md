...
- **Compass — Workspace Knowledge Graph**: Converts your workspace into a queryable knowledge graph via tree-sitter AST extraction across 12 languages (Python, JS/TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP). Claude queries the graph via 9 MCP tools (`query_graph`, `get_node`, `get_neighbors`, `shortest_path`, `god_nodes`, `get_community`, `graph_stats`, `compass_reindex`, `compass_status`) instead of reading full source files for structural questions — fewer tool calls, smaller conversation history, significant token savings (~71x compression for structural understanding). Louvain community detection groups related entities; god node analysis identifies core abstractions; surprising connection scoring surfaces non-obvious couplings. Disk-persisted extraction cache ensures fast incremental rebuilds on VS Code restart (~100-150ms vs 20-30s). Auto-reindex via file watcher with debounce. Graph terms optionally enhance recall orientation BM25 queries. Disabled by default; enable via `damocles.compass.enabled`
