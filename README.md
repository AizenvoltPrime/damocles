...
- **Context Distillation (Beta)**: Alternative context strategy that replaces the SDK's session resume with a per-session FTS5 database. Each turn runs as a stateless SDK session while Haiku observes the conversation and annotates structured entries via MCP tools (`list_prompt_entries`, `update_entry_description`, `write_prompt_summary`, `get_context_summary`). Entries are tracked by file path, entry type, tool calls, tags, and related files. Context retrieval uses FTS5 full-text search ranked by BM25 against the user's prompt, with a two-layer output: continuity (previous prompt's summary, always included) and relevant context (FTS-matched entries, budget-constrained). This keeps context usage predictable regardless of conversation length. Haiku fires a single observation call after each turn completes (including on user cancel). Per-panel strategy selection — each panel independently chooses `default` or `distill` via "This panel" and "Default for new panels" dropdowns in the settings panel. Subagent tool call details are persisted to per-agent JSONL files, enabling full subagent overlay in both live execution and history loading. The sparkles icon in the chat header opens the Haiku Observer overlay showing the observation per prompt with thinking, text output, and clickable MCP tool cards (opening full input/output overlays), prompt navigation, and buttons to open the raw log or context summary
