...
Files are **JSONC**. Both `//` line and `/* */` block comments are allowed. Scripts live by convention next to
...
> Damocles agent dir, not env, so they are **not** exposed. Any secret you keep in an ambient env var
> (e.g. `OPENAI_API_KEY`) still reaches the hook process. Don't pipe the environment to an untrusted command.
...
> any other truthy value are ignored and the turn continues. That is easy to hit from a shell or `jq`
...
| `agent_end` | the agent finished a turn, once per turn | observe-only |
| `subagent_end` | a subagent finished its run, once per subagent | observe-only; carries `parent_tool_use_id` |
...
`session_before_tree`, `turn_start`, `turn_end`, `agent_start`, `agent_before_settle`,
`message_start`, `message_end`, `resources_discover`.
...
> `agent_before_settle` fires after the agent run finishes and before pi settles it. pi lets a handler
> append session entries and request one more provider request there; Damocles dispatches it
> observe-only, so a hook on it can log or notify, and can neither append an entry nor ask for another
> provider request.

### Excluded (never spawned)

`message_update`, `tool_execution_start/update/end`, `context`, `context_with_system`,
`before_provider_request`, `after_provider_response`, `user_bash`, `project_trust`. All are too
high-frequency or mutation-heavy for a spawned command.
...
**Deny a tool** by emitting a `decision: "deny"` with a reason the model sees:

```python
print(json.dumps({"decision": "deny", "reason": "writes to /etc are not allowed"}))
```

**Deny and end the turn** by adding `terminate` when the model should stop rather than try another way. It is
...
**Rewrite a tool's input** by returning the uniform tool shape. Damocles maps it to the engine's native
shape before the tool runs:
...
- `transcript_path` points at the session JSONL (the canonical, complete record). The inline `messages`
  array on `agent_end` / `subagent_end` is a convenience snapshot of the conversation as the model sees
  it; for very long sessions, read `transcript_path`.
...
- **Hook didn't fire?** Check, in order: (1) the workspace is **trusted** if the hook is in the project
  ...
  (4) the **Damocles** output channel, where every hook run is logged with its event key, command, exit
  code, and decision.
