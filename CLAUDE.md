...
| `team/`               | Multi-agent teams via MessageBus + Scratchpad (disabled by default). Model/thinking policy in `pi-session/team-model-resolution.ts`: on Anthropic every agent is forced to Opus 4.8 with fixed thinking (lead/reviewer xhigh, implementor high); the `kind` spawn param sets thinking depth only. Per-agent tokens accumulate lifetime (`agent-runner.ts`, via `subagents/usage.ts`). Other backends unchanged. |
