...
- **Translator** (`openai-transform.ts`) — True event-by-event SSE; strips `x-anthropic-*` lines (neutralizes Claude Code CLI's rotating `cch` hash that defeats Codex's prompt cache), `cache_control` markers, and `thinking` blocks from inbound `messages[]`; tool-name normalization for >64-char MCP tools with reverse-mapping; no message-count or tool-count caps (length is governed solely by the 25 MB translated-body cap and the backend's natural `context_length_exceeded`). Auto-compaction is opt-in for every backend via the user's `damocles.autoCompact` setting (default off); GPT sessions are never force-compacted, so an unconfigured long GPT session can still surface a backend `context_length_exceeded` at the real window (clearer translation of that error is a possible follow-up).
...
**Compass** — Knowledge graph via tree-sitter AST → SQLite → Louvain → 8 MCP tools (incl. `compass_dead_code`). Disabled by default. Grammar WASM via `npm run fetch:grammars`. SQLite (sql.js-fts5) at `~/.damocles/compass/<workspace-hash>/graph.db`; `GraphStore.open()` self-heals a corrupt DB (discard + fresh rebuild — it's a regenerable cache). Module split: `database.ts` (GraphStore + path resolution + `runChunked` 400-param IN-chunker) with `db-wrapper.ts` / `import-resolver.ts` / `validation.ts` / `post-process.ts` extracted; git ops consolidated in `git.ts`; scheduler core in `worker-core.ts` (compass-worker.ts is the thin parentPort shell). **Transactions:** `withTransaction` is the only entry point; no `await` inside an open transaction; light queue strictly read-only; `schedulerYield` logs an invariant error + skips light drain if a transaction is somehow open. `DbWrapper` depth tracking excludes `END` (trigger DDL bodies end with `; END`) and exempts `temp.`-schema statements from the dirty flag; `serialize()` no-ops when clean. Schema v4: unique edge index `(kind, source_qualified, target_qualified, file_path, line)` + single-statement `INSERT … ON CONFLICT` upsert (no `RETURNING` — bundled SQLite 3.33). **Git paths** (`git.ts`): internally-parsed paths remap repo-root-absolute (`resolveRepoRoot`, success-only cache) → workspace-containment filter → re-anchor onto the caller's workspace spelling (win32 drive-case; `path.win32.relative` is case-insensitive); caller-supplied `changed_files` never remapped; `SAFE_GIT_REF` rejects leading `-`. **Incrementals:** watcher accumulates URIs through `createWatcherFileFilter` (noise/hidden dirs, sensitive, symlink + realpath root-escape; ENOENT passes so deletions flow through), burst >500 falls back to git diff; `mutated` gates resolve/TESTED_BY/metadata so no-op saves stay clean. **Worker lifecycle:** injectable `CompassWorkerFactory`; 3 consecutive failures → terminal `failed` (exponential backoff between; Rebuild resets); runLoop crash → `process.exit(1)` so the breaker sees it; validation fast-rejects busy during in-progress AND queued builds (`hasQueuedHeavy`). Extraction failures throw typed `GrammarLoadError` → recorded in `errors[]`, one warning per language per build; `analyzeChanges` caps at `MAX_CHANGED_FUNCS=500` with `truncated` flag. Blast radius: level-batched BFS (`getEdgesBySources`/`getEdgesByTargets`/`getNodesByQualifiedNames` on `runChunked`) — O(depth) statements, parity-tested vs legacy. `resolveExternalEdges` mirror-probe prefilter early-exits when only permanent externals remain (non-ASCII targets stay actionable — SQLite `LOWER()` is ASCII-only). Rust `#[test]`-attribute Test classification; `TESTED_BY` derived from CALLS-from-Test (`buildTestedByEdges`). FTS5 BM25 with `splitIdentifier()`; `search_aux` column indexes parent-class tokens + enclosing dir. Windows-safe path matching via `resolveGraphFilePaths` (exact + segment-anchored suffix, win32 case-fold). Non-lossy blast cap (explicit `truncated`). Louvain via graphology (deterministic; directory fallback >20K nodes). Cooperative scheduler: light reads preempt heavy builds at `scheduler.yield()`. Security: symlink skip, FTS5 query escaping, parameterized SQL. UI: D3 force graph, validation panel, tree view. Integrations: `UserPromptSubmit` injects `<damocles_compass>` XML; recall uses graph neighbors for BM25 expansion.
...
**SDK Integration** — `ClaudeSession` wraps SDK `query()` with `canUseTool` → PermissionHandler, lifecycle hooks, `stream_event` deltas. SDK dynamically imported (ESM from CJS). Custom `systemPrompt` replaces SDK's `claude_code` preset (drops auto-memory ~800 tokens, adds anti-verbosity rules); `tools` preset unchanged so built-in tools/agents still load. `buildThinkingOptions()` via `ModelInfo.supportsAdaptiveThinking`. `normalizeToolResult()` dual-path (live via `tool-manager.ts`, history via `history-manager.ts`). Prompt-index advances only in `sendMessage`; `{ isInternal: true }` skips advance for system-issued prompts. Remote messages reroute through `sendMessage` via UserPromptSubmit `decision: 'block'` so recall and non-recall share one counter site. History replay restamps from `node-turn-ref` JSONL (`uuid → {promptIndex, nodeId}`), falling back to a synthetic counter that advances only when `!isInjected`. Background-task chips cover any `run_in_background` tool (Bash shells + Agent subagents) via `ToolManager.backgroundToolUseIds` (turn-scoped, read at `task_started`); `backgroundTaskIds` is **not** turn-scoped — a fire-and-forget Bash ends its turn before its completion `task_notification` arrives, so ids persist across turns and are removed only on the terminal notification (`unregisterBackgroundTask`). Stop resolution is client-owned: on `stopTask` success the surface-aware handler emits a `stopped` terminal message (SDK 0.3.163+ resolves stop even when the task is already gone), and the store merge is idempotent (status first-wins, summary/usage enriched). Model refusals route `stop_reason:'refusal'` + `stop_details` through the `done` message (captured from `pendingAssistant`, guarded `!parentToolUseId` = main-chat only) → a dedicated `RefusalCard` (structured detection, never text-matched; live-session only).
