...
- **Per-call env sanitization (no `process.env` mutation):** `sdk-env.ts:buildSdkEnv()` returns a fresh sanitized env record on each call — shallow-copies `process.env`, strips `CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY`, pins `CLAUDE_CONFIG_DIR` to the Damocles dir. Every SDK spawn site passes the result explicitly via `options.env` (main chat `query-manager.ts:buildEnv()`, warmup, BTW, team agents, recall loop, recall sub-calls, haiku orientation, memory expansion). Never mutates the shared extension-host `process.env` — required because VS Code runs every extension in a single Node process, so a global write would leak into peers (e.g. the Claude Code VS Code extension would resolve its own config to `~/.damocles/auth/` and contend on our files)
